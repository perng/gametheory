         ...
         //window.onload = function() {
            ...
            socket.onclose = function(e) {
               ...
               if (current_cmd == 'doConnect') {
                   ...
                   handle_cr_logout('ok', 'connection closed by server', '0');
                   toggleButton(true);
                   cleanup_logout();
                   dispConnectionStatus("<font color='red'>Connection closed by server.</font>");
               }
            }
         };
...
": @rooms (list rooms), @who (list room users), @users (list logged in users), @create {room} (create and join a new room), @join {room} (join an existing room), @leave (leave a room), @where (show current room name), @public (set room as public), @private (set room as private), @master (assign another room user as master), @kick (kick a user out of current room), @max {max_size} (set room max size), @passwd {new password} (update password), @logout (logout)");
               ...
               // All commands below need a non-empty current_room.
                   ...
                   if (! is_room_master) {
                       appendChatroomInfo(msg + ': you have no permission for this operation');
                   }   
                   else {
                   ...
                   if (! is_room_master) {
                       appendChatroomInfo('@max: you have no permission for this operation');
                   }
                   else {
                       current_cmd = "max";
         ...
         function doRegister() {
             ...
             if (usr == '' || pwd == '') {
                 msg = 'Register failed: name and password cannot be empty.';
                 ...
                 msg = 'Register failed: two passwords do not match.';
                 //updateInfo(msg, 'error');
                 alert(msg);
         ...
         function doLogin() {
             ...
             if (usr == '' || pwd == '') {
                 msg = 'Login failed: name and password cannot be empty.';
                 alert(msg);
         ...
         function handle_c_response(jo) {
             ...
             else if (last_cmd == 'master')      { handle_cr_master(status, msg, tracker); }
             else if (last_cmd == 'kick')      { handle_cr_kick(status, msg, tracker); }
             else if (last_cmd == 'max')      { handle_cr_max(status, msg, tracker); }
             else if (last_cmd == 'set_room_permission') { handle_cr_set_room_permission(status, msg, tracker); }
             ...
             if (type == 'join_room') {
                 ...
                 if (v.length == 3) {
                     ...
                     if (new_master == current_user) {
                         is_room_master = true;
                         appendChatroomInfo(old_master + ' appointed you as the new room master.');
                     }
                 }
                 else if (DEBUG) {
                 ...
                 var room = v[2];
                 appendChatroomInfo('You are kicked out of the room ' + room);
             }
             else if (type == 'kick') {
                 var v = usr.split(':');  // kicked_user:room_master:room_name
                 var user = v[0];
                 appendChatroomInfo('user ' + user + ' is kicked out');
             }
             else if (type == 'max') {
                 var v = usr.split(':');  // max_size:room_master:room_name
                 var size = v[0];
                 appendChatroomInfo('Room max size is changed to ' + size + '.');
             }
         }
         ...
         function handle_cr_create_room(status, msg, tracker) {
             ...
             if (status != 'ok') {
                 //updateInfo(msg, 'error');
                 appendChatroomError('@create failed: ' +msg);
                 return;
             }
         ...
         function handle_cr_master(status, msg, tracker) {
             if (status != 'ok') {
                 appendChatroomInfo(msg);
                 return;
             }
             ...
             msg = "You set room master to " + msg + '.';
             appendChatroomInfo(msg);
             setRoomMaster(new_master, old_master, room_name);
             is_room_master = false; // current_user is no longer room master.
         }

         function handle_cr_kick(status, msg, tracker) {
             if (status != 'ok') {
                 appendChatroomInfo(msg);
                 return;
             }
             ...
             msg = 'User ' + user + ' is kicked out of this room.';
             appendChatroomInfo(msg);
         }

         function handle_cr_max(status, msg, tracker) {
             if (status != 'ok') {
                 appendChatroomInfo(msg);
                 return;
             }
         ...
         function handle_cr_set_room_permission(status, msg, tracker) {
             if (status != 'ok') {
                 appendChatroomInfo(msg);
                 return;
             }
             ...
             msg = "You set this room to " + (msg == '1' ? 'public' : 'private') + '.';
             appendChatroomInfo(msg);
         }
         ...
         function handle_cr_logout(status, msg, tracker) {
             ...
             if (status == 'ok') {
                 $('#div_logout').html('');
                 appendChatroomInfo('You are logged out.', 'info');
                 //$('#span_login').show();
                 //cleanup_logout();
                 getInputFocus();
             }
         ...
         $(document).ready(function() {
             ...
             $('#reg_name').watermark('Username');
             $('#reg_pwd').watermark('Password');
             $('#reg_pwd2').watermark('Password (repeat)');
